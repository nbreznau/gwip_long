---
title: "GWIP"
author: "Nate Breznau"
date: "`r Sys.Date()`"
output: html_document
---

Global Work-Injury Policy Database Longitudinal (GWIP_Long) v1

Sources:

Each individual indicator:

Welfare State Information Systems source: ([WeSIS](https://www.wesis.org/)) at the University of Bremen.

Harvard Dataverse source:
[GWIP_long](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/IVKYIE) Harvard Dataverse 

Enslaved Human Capital extraction source:
Nunn, Nathan. 2008. “The Long-Term Effects of Africa’s Slave Trades.” The Quarterly Journal of Economics 123(1):139–76. doi:10.1162/qjec.2008.123.1.139.



## Setup

```{r }

packages <- c("tidyverse",
              "pacman",
              "here",
              "ggplot2",
              "ragg",
              "knitr",
              "ggpubr",
              "cowplot")

pacman::p_load(packages, character.only = T)

```


## Data


```{r}

df <- read_csv(here("data", "df.csv"))

# set coverage before the first observation to zero

df <- df %>%
  mutate(
    cov_ii = if_else(is.na(cov_i), 0, cov_i)
  )


```

## Regions

### Encode

```{r}
df <- df %>%
  mutate(
    region = case_when(
      # North America
      iso3c %in% c("USA", "CAN", "PRI") ~ "North America",
      
      # Central America and Caribbean
      iso3c %in% c("MEX", "GTM", "BLZ", "HND", "SLV", "NIC", "CRI", "PAN",
                   "CUB", "DOM", "HTI", "JAM", "BRB", "TTO", "ATG", "BHS",
                   "DMA", "GRD", "KNA", "LCA", "VCT") ~ "Central America",
      
      # South America
      iso3c %in% c("ARG", "BRA", "CHL", "COL", "PER", "URY", "ECU", "BOL",
                   "PRY", "VEN", "GUY", "SUR") ~ "South America",
      
      # Western Europe
      iso3c %in% c("GBR", "FRA", "DEU", "NLD", "BEL", "CHE", "AUT", "IRL", "GRC", "CYP",
                   "DNK", "NOR", "SWE", "FIN", "LUX", "ISL", "ESP", "PRT",
                   "ITA", "AND", "MCO", "SMR", "LIE", "MLT") ~ "Western Europe",
      
      # Eastern Europe
      iso3c %in% c("POL", "CZE", "SVK", "HUN", "ROU", "BGR", "HRV", "SVN",
                   "EST", "LVA", "LTU", "RUS", "UKR", "BLR", "MDA", "SRB", "GEO", "AZE", "ARM",
                   "MKD", "BIH", "ALB", "MNE", "KOS") ~ "Eastern Europe",
      
      # Northern Africa
      iso3c %in% c("MAR", "DZA", "TUN", "LBY", "EGY", "SDN") ~ "Northern Africa",
      
      # Southern Africa
      iso3c %in% c("ZMB", "ZWE", "ZAF", "NAM", "BWA", "LSO", "SWZ") ~ "Southern Africa",
      
      # Sub-Saharan Africa 
      iso3c %in% c("BDI", "BEN", "BFA", "CIV", "CMR", "COD", "COG", "COM",
                   "CPV", "DJI", "ETH", "GAB", "GHA", "GIN", "GMB", "GNB",
                   "GNQ", "KEN", "LBR", "MDG", "MLI", "MOZ", "MRT", "MUS",
                   "MWI", "NER", "NGA", "RWA", "SEN", "SLE", "SOM", "SSD", "AGO", "CAF",
                   "STP", "SYC", "TCD", "TGO", "TZA", "UGA") ~ "Sub-Saharan Africa",
      
      # Middle East
      iso3c %in% c("ARE", "BHR", "IRN", "IRQ", "ISR", "JOR", "KWT", "LBN",
                   "OMN", "QAT", "SAU", "SYR", "TUR", "YEM") ~ "Middle East",
      
      # Central Asia
      iso3c %in% c("KAZ", "UZB", "TJK", "KGZ", "TKM", "AFG") ~ "Central Asia",
      
      # South Asia
      iso3c %in% c("BGD", "BTN", "IND", "LKA", "NPL", "PAK") ~ "South Asia",
      
      # South East Asia
      iso3c %in% c("IDN", "KHM", "LAO", "MMR", "MYS", "PHL", "SGP", "THA",
                   "TLS", "VNM", "BRN") ~ "South East Asia",
      
      # East Asia
      iso3c %in% c("CHN", "JPN", "KOR", "MNG", "PRK", "TWN") ~ "East Asia",
      
      # Oceania
      iso3c %in% c("AUS", "NZL", "FJI", "PNG", "SLB", "VUT", "WSM", "TON",
                   "KIR", "NRU", "TUV", "FSM", "PLW", "MHL") ~ "Oceania",
      
      TRUE ~ "Other"
    )
  )


```

### Collapse

Regional level by decade

```{r}

df <- df %>%
  mutate(
    decade = floor(year / 10) * 10  # bin years into decades
  )

# First collapse to have only one average per country
df_country <- df %>%
  group_by(iso3c, decade) %>%
  summarise(cov_ii = mean(cov_ii, na.rm = T),
            gen_i = mean(gen_i, na.rm = T),
            pop = mean(pop_i, na.rm = T),
            slave_trade_all = mean(slave_trade_all, na.rm = T)) %>%
  ungroup()

df_reg <- subset(df, year == 2020, select = c(colonial_history, region, iso3c))

df_country <- df_country %>%
  left_join(df_reg, by = "iso3c") %>%
  mutate(gen_ii = ifelse(gen_i == "NaN", 0, gen_i))

df_africa <- df_country %>%
  subset(region == "Sub-Saharan Africa" | region == "Southern Africa") %>%
  subset(decade > 1900) %>%
  ungroup()

df_region <- df_country %>%
  group_by(region, decade) %>%
  summarise(
    mean_cov_ii = mean(cov_ii, na.rm = TRUE),  
    pop = sum(pop, na.rm = TRUE), 
    .groups = "drop"
  )

df_colonial <- df_country %>%
  group_by(colonial_history, decade) %>%
  summarise(
    mean_cov_ii = mean(cov_ii, na.rm = TRUE), 
    se_cov_ii = sd(cov_ii, na.rm = TRUE) / sqrt(sum(!is.na(cov_ii))),
    mean_gen_ii = mean(gen_ii, na.rm = TRUE),
    se_gen_ii = sd(gen_ii, na.rm = TRUE) / sqrt(sum(!is.na(gen_ii))),
    pop = sum(pop, na.rm = TRUE), 
    .groups = "drop"
  )

```


### Correlation by decade

```{r}
# Compute log-transformed slave trade levels following Nunn
df_africa <- df_africa %>%
  mutate(log_slave_trade = log1p(slave_trade_all))  


cor_df <- df_africa %>%
  filter(decade > 1950) %>%
  group_by(decade) %>%
  summarise(
    cor_cov = cor(cov_ii, log_slave_trade, use = "complete.obs"),
    cor_gen = cor(gen_ii, log_slave_trade, use = "complete.obs")
  ) %>%
  mutate(
    label_cov = sprintf("%.2f", cor_cov),
    label_gen = sprintf("%.2f", cor_gen)
  )

```


## Population Estimates

```{r}

df_reg_pop <- subset(df_region, decade > 2011)

df_reg_pop <- df_reg_pop %>%
  mutate(pop_pct = round((pop/sum(df_reg_pop$pop))*100),2)

df_south_pop <- subset(df, year == 2020) %>%
  group_by(global_south) %>%
  mutate(gdp_notpc = gdp*pop) %>%
  summarise(
    pop = sum(pop_i, na.rm = T),
    gdp = sum(gdp_notpc, na.rm = T),
    cov = mean(cov_ii, na.rm = T),
    gen = mean(gen_i, na.rm = T)
  ) %>%
  ungroup() %>%
  mutate(gdppc = gdp/pop)

kableExtra::kable(df_south_pop)

```



## Fig 1. Coverage All Countries over Time

### Color, Appendix Only

```{r}

agg_png(here("results", "Fig1.png"), res = 144, height = 1000, width = 750)

ggplot(df_region, aes(x = decade, y = mean_cov_ii, color = region, group = region)) +
  geom_line(size = 1.2) +
  geom_text(data = df_region %>% group_by(region) %>% filter(decade == max(decade)),
            aes(label = region),
            hjust = -0.1,
            vjust = c(0.8, # C Amer
                      -0.5, # C Asia
                      0.2, # E Asia
                      -0.1, # E Eur
                      -0.6, # M East
                      0, # N Am
                      0.4, # N Afric
                      0.3, # Oceana
                      -0.2, # S Am
                      0, # S Asia
                      1, #SE Asia
                      0,
                      0,
                      0 # W Eur
                      ),
            size = 3,
            show.legend = FALSE) +
  scale_x_continuous(
    limits = c(1870, max(df_region$decade, na.rm = T) + 40),
    breaks = seq(1880, 2020, by = 20),
    expand = expansion(mult = c(0.01, 0.15))
  ) +
  theme_classic() +
  theme(
    legend.position = "none",  # no legend, use direct labels
    plot.title = element_text(hjust = 0.5),
    plot.margin = margin(10,70,10,10)
  ) +
  labs(
    title = "Average Work-Injury Policy Coverage by Region Over Time",
    x = "Decade",
    y = "Coverage (Avg. %)"
  ) 

dev.off()

include_graphics(here("results", "Fig1.png"))


```

### Greyscale, Book Chapter

```{r}

agg_png(here("results", "Fig1_grey.png"), res = 144, height = 1000, width = 1000)

ggplot(df_region, aes(x = decade, y = mean_cov_ii, color = region, group = region, linetype = region)) +
  geom_line(size = 1.2) +
  geom_text(data = df_region %>% group_by(region) %>% filter(decade == max(decade)),
            aes(label = region),
            hjust = -0.1,
            vjust = c(0.8, # C Amer
                      -0.5, # C Asia
                      0.2, # E Asia
                      -0.1, # E Eur
                      -0.6, # M East
                      0, # N Am
                      0.4, # N Afric
                      0.3, # Oceana
                      -0.2, # S Am
                      0, # S Asia
                      1, #SE Asia
                      0,
                      0,
                      0 # W Eur
                      ),
            size = 3.2,
            show.legend = FALSE) +
  scale_x_continuous(
    limits = c(1870, max(df_region$decade, na.rm = T) + 40),
    breaks = seq(1880, 2020, by = 20),
    expand = expansion(mult = c(0.01, 0.15))
  ) +
  scale_color_manual(values = c(
    "Central America" = "grey42",
    "Central Asia"   = "grey35",    
    "East Asia"       = "grey30",   
    "Eastern Europe"   = "grey37", 
    "Middle East"      = "grey22",  
    "North America"    = "grey19",  
    "Northern Africa"   = "grey13", 
    "Oceania"           = "grey19",
    "South America"      = "grey10",
    "South Asia"         = "grey45",
    "South East Asia"    = "grey38",
    "Southern Africa"   = "grey29",
    "Sub-Saharan Africa" = "grey23",
    "Western Europe" = "grey35"
  )) +
  scale_linetype_manual(values = c(
    "Central America" = "dashed",
    "Central Asia" = "solid",
    "East Asia" = "solid",
    "Eastern Europe" = "dashed",
    "Middle East" = "dotted",
    "North America" = "dashed",
    "Northern Africa" = "solid",
    "Oceania" = "dotted",
    "South America" = "dashed",
    "South Asia" = "solid",
    "South East Asia" = "dashed",
    "Southern Africa" = "dashed",
    "Sub-Saharan Africa" = "dotted",
    "Western Europe" = "solid"
  )) +
  theme_classic() +
  theme(
    legend.position = "none",  # no legend, use direct labels
    plot.title = element_text(hjust = 0.5),
    plot.margin = margin(10,70,10,10)
  ) +
  labs(
    title = "Average Work-Injury Policy Coverage by Region Over Time",
    x = "Decade",
    y = "Coverage (Avg. %)"
  )

dev.off()

include_graphics(here("results", "Fig1_grey.png"))

```

## Fig 2. Cov and Gen British/English Colonies

### Color Version, Appendix Only

#### Prep

```{r}

# SE bands don’t go below 0, better visually
df_plot <- df_colonial %>%
  subset(!colonial_history %in%  c("Various", "Germany", "Belgium", "Netherlands", "Portugal")) %>%
  mutate(
    cov_ii_lower = pmax(0, mean_cov_ii - se_cov_ii),
    cov_ii_upper = mean_cov_ii + se_cov_ii,
    gen_ii_lower  = pmax(0, mean_gen_ii - se_gen_ii),
    gen_ii_upper  = mean_gen_ii + se_gen_ii
  )
```

#### Plot

```{r}

agg_png(here("results", "Fig2.png"), res = 144, height = 800, width = 800)

# Plot
ggplot() +
  # cov_ii: solid lines with SE ribbon
  geom_ribbon(
    data = df_plot,
    aes(x = decade, ymin = cov_ii_lower, ymax = cov_ii_upper, group = colonial_history),
    fill = "grey80", alpha = 0.4
  ) +
  geom_line(
    data = df_plot,
    aes(x = decade, y = mean_cov_ii, color = colonial_history, group = colonial_history),
    size = 1
  ) +

  # gen_ii: dashed lines with SE ribbon
  geom_ribbon(
    data = df_plot,
    aes(x = decade, ymin = gen_ii_lower, ymax = gen_ii_upper, group = colonial_history),
    fill = "grey80", alpha = 0.4
  ) +
  geom_line(
    data = df_plot,
    aes(x = decade, y = mean_gen_ii, color = colonial_history, group = colonial_history),
    size = 1, linetype = "dashed"
  ) +
  annotate("text",
         x = 2000, y = 7,
         label = "Coverage\n(solid lines)",
         size = 3.5, color = "grey25") +
  annotate("text",
         x = 1930, y = 80,
         label = "Replacement Rate\n(dashed lines)",
         size = 3.5, color = "grey25") +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_x_continuous(
    limits = c(1870, NA),
    expand = expansion(mult = c(0.01, 0.15))
  ) +
  labs(
    x = "Decade",
    y = "Percentage Score",
    color = "Colonial History",
    title = "Trends in Coverage and Generosity by Colonial History"
  ) +
  theme_classic()

dev.off()

include_graphics(here("results", "Fig2.png"))


```
### Greyscale, Book Chapter

#### Prep

```{r}
# Define line styles
line_styles <- c(
  "France" = "solid",
  "UK" = "dashed",
  "Spain" = "dotted",
  "Russian" = "dotdash",
  "None" = "longdash"
)


df_plot$colonial_history <- factor(df_plot$colonial_history, levels = names(line_styles))


p1 <- ggplot(df_plot, aes(x = decade, y = mean_cov_ii, group = colonial_history)) +
  geom_ribbon(aes(ymin = cov_ii_lower, ymax = cov_ii_upper), fill = "grey80", alpha = 0.4) +
  geom_line(aes(linetype = colonial_history), size = 1, color = "black") +
  scale_linetype_manual(values = line_styles) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_x_continuous(limits = c(1870, NA), expand = expansion(mult = c(0.01, 0.15))) +
  labs(x = "", y = "Percentage Score", linetype = "Colonial History", title = "Coverage") +
  theme_classic() +
  theme(
  legend.position = "bottom",
  legend.key.width = unit(1.5, "cm"),  # Increase line length in legend
  axis.title.x = element_blank(),
  axis.title.y = element_blank()
)

p2 <- ggplot(df_plot, aes(x = decade, y = mean_gen_ii, group = colonial_history)) +
  geom_ribbon(aes(ymin = gen_ii_lower, ymax = gen_ii_upper), fill = "grey80", alpha = 0.4) +
  geom_line(aes(linetype = colonial_history), size = 1, color = "black") +
  scale_linetype_manual(values = line_styles) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_x_continuous(limits = c(1870, NA), expand = expansion(mult = c(0.01, 0.15))) +
  labs(x = "", y = "Percentage Score", linetype = "Colonial History", title = "Replacement Rate") +
  theme_classic() +
  theme(
  legend.position = "bottom",
  legend.key.width = unit(1.5, "cm"),
  axis.title.x = element_blank(),
  axis.title.y = element_blank()
)


final_plot <- ggarrange(
  p1, p2,
  ncol = 2,
  common.legend = TRUE,
  legend = "bottom"
)
```

#### Plot

```{r}

agg_png(here("results", "Fig2_grey.png"), res = 144, height = 800, width = 1200)
print(final_plot)
dev.off()


include_graphics(here("results", "Fig2_grey.png"))

```


## Fig 3. African Slave Trade and GWIP

### Setup Plots

#### Color Version, Appendix Only

```{r}

plot_cov <- ggplot(df_africa, aes(x = decade, y = cov_ii, group = iso3c, color = log_slave_trade)) +
  geom_line(size = 0.8, alpha = 0.8) +
  scale_color_gradient2(low = "#FDE725", 
                        mid = "#1F9E89", 
                        high = "#3E4989", 
                        midpoint = 7, name = "Log Slave Trade") +
  scale_y_continuous(labels = scales::percent_format(scale = 1),
                     limits = c(-6, NA)) +  
    scale_x_continuous(breaks = seq(1880, 2020, by = 20)) +

  labs(
    x = "Decade",
    y = "Labor Force Coverage (%)"
  ) +
  geom_text(
    data = cor_df,
    aes(x = decade, y = -4.5, label = label_cov),
    inherit.aes = FALSE,
    size = 3.5,
    hjust = 0.5,
    angle = 45,
    color = "grey20"
  ) +
  annotate("text", x = 1932, y = -4.5, label = "Correlation = ", color = "grey20") +
  theme_classic() + 
  theme(legend.position = "none")

plot_gen <- ggplot(df_africa, aes(x = decade, y = gen_ii, group = iso3c, color = log_slave_trade)) +
  geom_line(size = 0.8, alpha = 0.8) +
  scale_color_gradient2(low = "#FDE725", 
                        mid = "#1F9E89", 
                        high = "#3E4989", 
                        midpoint = 7, name = "Log Slave Trade") +
  scale_y_continuous(labels = scales::percent_format(scale = 1),
                     limits = c(-6, NA)) +
  scale_x_continuous(breaks = seq(1880, 2020, by = 20)) +

  labs(
    x = "Decade",
    y = "Replacement Rate (%)"
  ) +
  geom_text(
    data = cor_df,
    aes(x = decade, y = -4.5, label = label_gen),
    inherit.aes = FALSE,
    size = 3.5,
    hjust = 0.5,
    angle = 45,
    color = "grey20"
  ) +
  theme_classic() +
  theme(legend.position = "none")


```

#### Greyscale, Book Chapter

```{r}

plot_cov_grey <- ggplot(df_africa, aes(x = decade, y = cov_ii, group = iso3c, color = log_slave_trade)) +
  geom_line(size = 0.8, alpha = 0.8) +
  scale_color_gradient2(low = "grey75", 
                        mid = "grey45", 
                        high = "grey10", 
                        midpoint = 7, name = "Log Slave Trade") +
  scale_y_continuous(labels = scales::percent_format(scale = 1),
                     limits = c(-6, NA)) +  
    scale_x_continuous(breaks = seq(1880, 2020, by = 20)) +

  labs(
    x = "Decade",
    y = "Labor Force Coverage (%)"
  ) +
  geom_text(
    data = cor_df,
    aes(x = decade, y = -4.5, label = label_cov),
    inherit.aes = FALSE,
    size = 3.5,
    hjust = 0.5,
    angle = 45,
    color = "grey20"
  ) +
  annotate("text", x = 1932, y = -4.5, label = "Correlation = ", color = "grey20") +
  theme_classic() + 
  theme(legend.position = "none",
        axis.title.x = element_blank())

plot_gen_grey <- ggplot(df_africa, aes(x = decade, y = gen_ii, group = iso3c, color = log_slave_trade)) +
  geom_line(size = 0.8, alpha = 0.8) +
  scale_color_gradient2(low = "grey80", 
                        mid = "grey55", 
                        high = "grey25", 
                        midpoint = 7, name = "Log Slave Trade") +
  scale_y_continuous(labels = scales::percent_format(scale = 1),
                     limits = c(-6, NA)) +
  scale_x_continuous(breaks = seq(1880, 2020, by = 20)) +

  labs(
    x = "Decade",
    y = "Replacement Rate (%)"
  ) +
  geom_text(
    data = cor_df,
    aes(x = decade, y = -4.5, label = label_gen),
    inherit.aes = FALSE,
    size = 3.5,
    hjust = 0.5,
    angle = 45,
    color = "grey20"
  ) +
  theme_classic() +
  theme(legend.position = "none",
        axis.title.x = element_blank())


```



### Plot Arrange

#### Color

```{r}
agg_png(here("results", "Fig3.png"), res = 144, height = 750, width = 1200)

ggarrange(plot_cov, plot_gen,
          common.legend = T,
          legend = "bottom") 

dev.off()

include_graphics(here("results", "Fig3.png"))

```

#### Grey

```{r}
agg_png(here("results", "Fig3_grey.png"), res = 144, height = 1000, width = 1000)

ggarrange(plot_cov_grey, plot_gen_grey, ncol = 1, nrow = 2,
          common.legend = T,
          legend = "bottom") 

dev.off()

include_graphics(here("results", "Fig3_grey.png"))

```
## Citations

```{r cites}

packages %>%
  map(citation) %>%
  print(style = "text")

```

## Colophon

```{r colo}

sessionInfo()

```

